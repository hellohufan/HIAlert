...
//  HIAlert
//
//  Created by hellohufan on 06/16/2020.
...
class ViewController: UIViewController {
    ...
    private func alertViewShowType(tag: Int) {
        ...
        let action_two = HIAlertAction(title: "知道了", style: .defaulted) { (action) in
...
//            UITextField
            let textView = HIAlertView(style: .textField, title: "自定义", message: "自定义的样式")
            ...
            let action_t = HIAlertAction(title: "测试3个按钮", style: .defaulted) { (action) in
                print("输入框的文字是----\(textView.textField.text ?? "")")
            }
            ...
            let animV = HIAlertView(style: .textField, title: "请输入密码", message: nil)
            ...
            let action_two = HIAlertAction(title: "确定", style: .defaulted) { (action) in
                animV.actions = []
                animV.resetActions()
                
                let animation = UIView(frame: CGRect(x: 0, y: 0, width: 80, height: 80))
                let anim = HIBeginLineAnimation.initShow(in: animation.bounds, lineWidth: 4, lineColor: UIColor.blue)
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    ...
                    let fail = HILineFailAnimation.initShow(in: animation.bounds, lineColor: UIColor.red)
                    animation.addSubview(fail)
                    
                    let cancel = HIAlertAction(title: "取消", style: .cancel) { (action) in
                        animV.dismiss()
                    }
    ...
    private func uiAlertViewShowType(tag: Int) {
        let view = HIUIAlertView(style: .defaulted)
        var title = "默认"
        var message = "内容默认的样式"
        let action_one = HIAlertAction(title: "取消", style: .cancel) { (action) in
            view.dismiss()
        ...
        if tag == 1 {
            ...
            let textView = HIUIAlertView(style: .textField, title: "自定义", message: "自定义的样式")
            textView.textField.placeholder = "自定义"
            let action_one = HIAlertAction(title: "取消", style: .cancel) { (action) in
                textView.dismiss()
            }
            ...
            let animV = HIUIAlertView(style: .textField, title: "请输入密码", message: nil)
            animV.textField.placeholder = "密码"
            ...
            let action_two = HIAlertAction(title: "确定", style: .defaulted) { (action) in
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    anim.paused()
                    anim.removeFromSuperview()
                    let fail = HILineSuccessAnimation.initShow(in: animation.bounds, lineColor: UIColor.red)
                    animation.addSubview(fail)
                    
                    let cancel = HIAlertAction(title: "成功了", style: .defaulted) { (action) in
                        animV.dismiss()
                    }
                    animV.addAction(action: cancel)
                    animV.resetActions()
...
// MARK: - 如果HIAlertView 和 HIUIAlertView  都无法满足需求 可以使用HIBaseAlertView
class baseAlertView: HIBaseAlertView {
