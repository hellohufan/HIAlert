...
/// style
public enum HIUIAlertViewStyle {
    case defaulted,
    textField
}

open class HIUIAlertView: UIView {
    ...
    /// 添加action
    public func addAction(action: HIAlertAction) {
    ...
    public func addButton(button: HIAlertViewButton) {
    ...
    /// action
    public var actions: [HIAlertAction] = []
    public var buttons: [HIAlertViewButton] = []
    ...
    /// 两个按钮时 排列方式
    public var actionArrangementMode: HIButtonArrangementMode = .horizontal {
        didSet {
    ...
    public convenience init(style: HIUIAlertViewStyle) {
        self.init(style: style, title: nil, message: nil)
    }
    
    public init(style: HIUIAlertViewStyle, title: String?, message: String?) {
        let frame = CGRect(x: 0, y: 0,
                       width: UIScreen.main.bounds.size.width,
                       height: UIScreen.main.bounds.size.height)
        super.init(frame: frame)
        
        alertView = HIBaseAlertView(frame: frame)
    ...
    public var alertView: HIBaseAlertView!
}

extension HIUIAlertView {
...
extension HIUIAlertView {
    ...
    private func createActions() {
        ...
        var alertButtons: [HIAlertViewButton] = []
        for (idx, action) in actions.enumerated() {
            let btn = HIAlertViewButton(type: .custom)
            btn.tag = idx
    ...
    @objc private func actionPressed(button: HIAlertViewButton) {
