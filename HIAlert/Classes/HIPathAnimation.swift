...
// MARK: 路径动画 旋转
public class HIBeginLineAnimation: UIView {
    ...
    public class func initShow(in frame: CGRect, lineWidth: CGFloat = 4, lineColor: UIColor? = UIColor.blue, layerSize: CGSize = CGSize.zero) -> HIBeginLineAnimation {
        let begin = HIBeginLineAnimation(frame: frame)
...
extension HIBeginLineAnimation {
    private func config() {
...
// MARK: - 成功动画 ✔️
/// 成功动画 ✔️
public class HILineSuccessAnimation: UIView {
    ...
    public class func initShow(in frame: CGRect, lineWidth: CGFloat = 4, lineColor: UIColor? = UIColor.blue, layerSize: CGSize = CGSize.zero) -> HILineSuccessAnimation {
        let success = HILineSuccessAnimation(frame: frame)
    ...
    public class func initConfig(in frame: CGRect, lineWidth: CGFloat = 4, lineColor: UIColor? = UIColor.blue, layerSize: CGSize = CGSize.zero, signWidth: CGFloat = 4, signColor: UIColor? = UIColor.blue) -> HILineSuccessAnimation {
        let success = HILineSuccessAnimation(frame: frame)
        success.lineWidth = lineWidth
        success.lineColor = lineColor
...
extension HILineSuccessAnimation: CAAnimationDelegate {
    private func config() {
        animationLayer.bounds = CGRect(x: 0, y: 0, width: bounds.size.width, height: bounds.size.height)
        animationLayer.position = CGPoint(x: bounds.size.width / 2, y: bounds.size.height / 2)
        layer.addSublayer(animationLayer)
...
// MARK: - 线条动画 失败 ❌
/// 动画 失败 ❌
public class HILineFailAnimation: UIView {
    ...
    /// - Parameter lineColor: 线条颜色
    /// - Parameter layerSize: 动画区域 默认 60
    public class func initShow(in frame: CGRect, lineWidth: CGFloat = 4, lineColor: UIColor? = UIColor.blue, layerSize: CGSize = CGSize.zero) -> HILineFailAnimation {
        let success = HILineFailAnimation(frame: frame)
        success.lineWidth = lineWidth
        success.lineColor = lineColor
        success.signWidth = lineWidth
        success.signColor = lineColor
    ...
    /// - Parameter signWidth: ❌的宽度
    /// - Parameter signColor: ❌的颜色
    public class func initConfig(in frame: CGRect, lineWidth: CGFloat = 4, lineColor: UIColor? = UIColor.blue, layerSize: CGSize = CGSize.zero, signWidth: CGFloat = 4, signColor: UIColor? = UIColor.blue) -> HILineFailAnimation {
        let success = HILineFailAnimation(frame: frame)
        success.lineWidth = lineWidth
        success.lineColor = lineColor
        success.signWidth = signWidth
...
extension HILineFailAnimation: CAAnimationDelegate {
    private func config() {
        animationLayer.bounds = CGRect(x: 0, y: 0, width: bounds.size.width, height: bounds.size.height)
        animationLayer.position = CGPoint(x: bounds.size.width / 2, y: bounds.size.height / 2)
        layer.addSublayer(animationLayer)
